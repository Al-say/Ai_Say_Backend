...
- **异步智能评估**: TaskID + 轮询模式，支持长时间 AI 评估（新增）
- **智能语音识别**: 集成百度 ASR 实现精准语音转文字
- **AI 智能评价**: DeepSeek 大模型提供个性化口语评价（支持 6 维雷达图评分）
...
- **评估模块（新）** (`/api/v1/evaluate`): 异步任务提交、状态查询、结果获取
- **主页模块** (`/api/home`): 每日挑战题目获取
- **传统评估** (`/api/eval`): 文本评估、音频评估、完整音频评估流程
